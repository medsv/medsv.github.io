...
def calc_p_st(t, RH, p = p_0):
    ...
    p: давление воздуха, Па (по умолчанию - нормальное атмосферное давление)
    """
    return calc_p_st_d(calc_d(t, RH, p), p)
...
def calc_p_cond_t_dp(t_dp, t, p = p_0):
    ...
    return calc_p_cond(d, t, p)


def calc_p_cond(d, t, p = p_0):
    """
    Определение давления начала конденсации содержащегося в воздухе водяного пара
    ...
    p_st = calc_p_st_d(d, p)  # парциальное давление водяного пара
    k = p_s / p_st
    return k * p

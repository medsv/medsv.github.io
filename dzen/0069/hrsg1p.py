...
Для Моделирование котла-утилизатора нескольких давлений необходимо рассматривать каждый
контур как отдельный котёл-утилизатор 
...
__author__ = "Медведев Сергей Владимирович"
__copyright__ = "Медведев Сергей Владимирович, 2023"
__license__ = "GPL"
__version__ = "1.1"
__maintainer__ = "Медведев Сергей Владимирович"
...
class HRSG1p():
    """
    Моделирование теплообмена в контуре низкого давления КУ двух давлений
    В данном контуре экономайзер нагревает воду контура высокого давления
    """
    def __init__(self, G_уг, t_уг, p_пп=1e6, t_к=45, dt_уг=30., dt_и=10., cp_уг=1037, dps=[0.,0.,0.], fi_ку = 0.995, dD_в_эк = 0):
        """
        ...
        dD_в_эк: вода, нагреваемая в экономайзере для другого потребителя, кг/с
        """
        self.G_уг = G_уг
        self.t_уг = t_уг
        self.p_пп = p_пп
        self.dD_в_эк = dD_в_эк
        self.t_к = t_к
        self.dt_уг = dt_уг
        self.dt_и = dt_и
        self.cp_уг = cp_уг
    ...
    def calc(self):
        ...
        #self.Qs[3] = self.D_пв * (self.hs_пв[0] - self.hs_пв[3]) / self.fi_ку - версия 1.0
        self.Qs[3] = (self.D_пв * (self.hs_пв[0] - self.hs_пв[3]) + self.dD_в_эк * (self.hs_пв[2] - self.hs_пв[3])) / self.fi_ку 
